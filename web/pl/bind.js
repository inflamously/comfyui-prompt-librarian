...
import { warnOnce } from "./dom.js";

/** The widget carrying the prompt body. Also the node's output. */
export const TEXT_WIDGET = "text";

/** The widget carrying the library record link. Hidden by pl_librarian.js. */
...
/* --------------------------------------------------------------------------
   Widget + element lookup
   -------------------------------------------------------------------------- */

/** Same guarded lookup as pl_librarian.js — `node.widgets` may not exist yet. */
 ...
 *
 * Modelled on the identical helper in pickers.js, which is not exported;
 * copying three lines beats coupling the binder to the picker module.
 */
 ...
 *
 * This is the widget-poking half of modal.js's `loadIntoNode`, extracted so
 * the live binding and the explicit `Load into node` button take exactly the
 * same path — two code paths writing the same widget is how they drift.
 *
 * ORDER MATTERS, and the reason is documented in modal.js: assign `.value`
 * FIRST, then invoke the callback. ComfyUI's own widget callbacks are not
 * trustworthy (the old node's file records one that "resets widget state and
 * fights our async update"), so the value is committed before one can run and
 * every invocation is individually wrapped.
 *
 * @param {object} node
...
/**
 * The backstop. Call from an existing timer — modal.js already runs a 1 s
 * heartbeat for target resolution, so this costs one string compare per second
 * and no new timer.
 *
