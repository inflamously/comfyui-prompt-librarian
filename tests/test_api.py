...
class MultipartField:
    """Chunked stand-in for aiohttp's multipart body-part reader."""
...
class StreamResponse:
    """In-memory stand-in for aiohttp's StreamResponse."""
...
def test_keep_both_does_not_shrink_the_badge(call, store):
    ...
    assert {scan["counts"][pid] for pid in ids} == {3}
...
def test_dupes_all_coalesces_concurrent_callers(call, store, monkeypatch):
    ...
    async def _race():
        return await asyncio.gather(*[api.queries._all_pairs(0.9, False) for _ in range(5)])
...
def test_update_expect_updated_conflict_is_409(call, store):
    ...
    status, payload = call(
        ...
        body={
            "id": rec["id"],
            "body": "three",
            "expect_updated": "1999-01-01T00:00:00Z",
        },
    )
    assert status == 409
...
def test_dupes_ignore_round_trip(call, store):
    ...
    payload = ok(
        call("post", "/dupes/ignore", body={"a": first["id"], "b": second["id"], "unignore": True})
    )
...
def test_snippet_set_and_delete(call, store):
    payload = ok(call("post", "/snippet", body={"op": "set", "name": "cine", "body": "35mm"}))
...
def test_settings(call, store):
    payload = ok(call("post", "/settings", body={"dupe_threshold": 0.8, "version_cap": 10}))
...
def test_storage_status_and_manual_compaction(call, store):
    status = ok(call("get", "/storage"))["storage"]
...
def test_explicit_legacy_migration_route(call, store, tmp_path, monkeypatch):
    legacy = tmp_path / "legacy-library.json"
    legacy.write_text(
        json.dumps(
            {
                "schema": 1,
                "settings": {"dupe_threshold": 0.2, "version_cap": 4},
                "prompts": [{"id": "legacy", "body": "from the old library"}],
            }
        ),
        encoding="utf-8",
    )
...
def test_streamed_file_export_is_the_portable_envelope(routes, store, monkeypatch):
    ...
    monkeypatch.setattr(
        api.routes.library,
        "_get_web",
        lambda: types.SimpleNamespace(StreamResponse=StreamResponse),
    )
    handler = routes.table[("get", api.PREFIX + "/export/file")]
    response = asyncio.run(handler(Request()))
    library = json.loads(response.body.decode("utf-8"))

    assert response.prepared is True
    assert response.headers["Content-Disposition"].startswith("attachment;")
    assert set(library) == {"schema", "updated", "settings", "snippets", "ignored", "prompts"}
    assert library["schema"] == 1
    assert library["prompts"] == [rec]
    assert library["snippets"]["intro"]["body"] == "35mm"
    assert library["ignored"] == [sorted([rec["id"], "ghost"])]
...
def test_multipart_file_import_merges_and_cleans_its_spool(routes, store, tmp_path, monkeypatch):
    ...
    incoming = json.dumps(
        {
            "schema": 1,
            "prompts": [{"id": "uploaded", "body": "from upload"}],
        }
    ).encode("utf-8")
    ignored = MultipartField("unrelated", b"ignored")
    request = MultipartRequest(
        [ignored, MultipartField("library", incoming)],
        query={"mode": "merge"},
    )
    monkeypatch.setattr(api.routes.library.tempfile, "tempdir", str(tmp_path))

    handler = routes.table[("post", api.PREFIX + "/import/file")]
    response = asyncio.run(handler(request))
...
def test_multipart_file_import_replace_and_bad_mode(routes, store, tmp_path, monkeypatch):
    ...
    incoming = json.dumps(
        {
            "schema": 1,
            "prompts": [{"id": "replacement", "body": "keep me"}],
        }
    ).encode("utf-8")
    monkeypatch.setattr(api.routes.library.tempfile, "tempdir", str(tmp_path))
    handler = routes.table[("post", api.PREFIX + "/import/file")]
    ...
    replaced = asyncio.run(
        handler(
            MultipartRequest(
                [MultipartField("library", incoming)],
                query={"mode": "replace"},
            )
        )
    )
    ...
    payload = json.loads(rejected.body.decode("utf-8"))
    assert rejected.status == 400 and payload["code"] == "bad_request"
    assert not list(tmp_path.glob("prompt-librarian-import-*.json"))
