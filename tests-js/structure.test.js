...
test("modal paths and architecture references stay consistent", () => {
  assert.equal(fs.existsSync(path.join(WEB, "prompt_librarian", "prompt_modal")), false);
  const oldPath = /(?:^|[^\w])prompt_modal\//;
  ...
  assert.deepEqual(files, allModules().filter(p => p.startsWith("prompt_librarian/modal/")).map(p => "web/" + p));
